...
export default function page() {
  ...
  const updatePayment = async (id, paymentStatus) => {
    try {
      const { data } = await axios.patch(`/api/enrollments/${id}`, {
        paymentStatus,
      });
      ...
      if (data.success) {
        ...
        setEnrollments((prev) =>
          prev.map((item) =>
            item._id === id ? { ...item, paymentStatus } : item
          )
        );
      } else {
        toast.error(data.message);
  ...
  const updateStatus = async (id, enrollmentStatus) => {
    try {
      const { data } = await axios.patch(`/api/enrollments/${id}`, {
        enrollmentStatus,
      });
      ...
      if (data.success) {
        ...
        setEnrollments((prev) =>
          prev.map((item) =>
            item._id === id ? { ...item, enrollmentStatus } : item
          )
        );
      } else {
        toast.error(data.message);
      ...
      toast.error(
        error.response?.data?.message || "Unable to update enrollment status."
      );
    }
  };
              ...
              {enrollments.length ? (
                enrollments.map((item) => (
                  <tr key={item._id} className="border-t border-gray-200">
                    ...
                    <td className="p-4">{item.course?.title || "N/A"}</td>
                    ...
                    <td className="p-4">
                      <select
                        ...
                        onChange={(e) =>
                          updatePayment(item._id, e.target.value)
                        }
